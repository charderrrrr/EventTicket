// TicketRepository - репозиторий для работы с таблицей tickets.
// Create - создает новый билет с привязкой к пользователю, событию и месту.
// GetById - возвращает билет по идентификатору или null.
// GetByUserId - возвращает все билеты пользователя, отсортированные по дате покупки (новые сверху).
// GetBySeatId - возвращает активный билет для конкретного места (проверка на двойную продажу).
// UpdateStatus - изменяет статус билета (Active, Refunded, Cancelled).
// Delete - физическое удаление билета при возврате.
// GetActiveByEventId - возвращает все активные билеты для события.
