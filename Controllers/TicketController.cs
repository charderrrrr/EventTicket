// TicketController - API контроллер управления билетами.
// Обрабатывает запросы на покупку билета, возврат билета с расчетом комиссии,
// получение списка билетов пользователя и предварительный расчет комиссии за возврат.
// При покупке проверяет доступность места и рассчитывает актуальную цену.
// При возврате удаляет билет и освобождает место.
